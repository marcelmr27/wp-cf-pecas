...
/**
 * Template Name: Catálogo Lançamentos/Promoções
 */
global $wp;
...
if ((int) url_to_postid(home_url($wp->request)) === 91) {
    $array_query = array(
        'key' => 'newest',
        'value' => 'Sim',
        'compare' => '='
                    ...
                    <div class="table-responsive">
                        <table>
                            <thead>
                                ...
                                <th>PRODUTO</th>
                                <?php if ((int) url_to_postid(home_url($wp->request)) !== 91) { ?>
                                    <th>DE</th>
                                    <th>POR</th>
                                <?php } ?>
                                <th>#</th>
                                ...
                                <?php if (!empty($array_produtos)) { ?>
                                    <?php foreach ($array_produtos as $PRODUTOS) { ?>
                                        <tr>
                                            ...
                                            <?php if ((int) url_to_postid(home_url($wp->request)) !== 91) { ?>
                                                <td><?php echo 'R$ ' . number_format(floatval($PRODUTOS['original_price']), 2, ',', '.') ?></td>
                                                <td><?php echo 'R$ ' . number_format(floatval($PRODUTOS['special_price']), 2, ',', '.') ?></td>
                                            <?php } ?>
